            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select name="uuid_dosen" class="form-select" data-control="select2" id="from_select_dosen"
                    ...
                    <div class="mb-10">
                        ...
                        <select name="jenis_bimbingan" class="form-select" data-control="select2" id="from_select_jenis"
                            data-placeholder="Pilih jenis inputan">
                        </select>
                        <small class="text-danger jenis_bimbingan_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Pembimbing</label>
                        <select name="pembimbing" class="form-select" data-control="select2" id="from_select_pembimbing"
                            data-placeholder="Pilih jenis inputan">
                        </select>
                        <small class="text-danger pembimbing_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'List Bimbingan');
        });
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/add-bimbingan', 'Tambah',
                    'List Bimbingan',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/update-bimbingan/' + uuid,
                    'Update',
                    'List Bimbingan', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/show-bimbingan/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'List Bimbingan', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-bimbingan/' + $(this).attr('data-uuid');
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="List Bimbingan" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        function push_select_dosen(url, element) {
            $.ajax({
                url: url,
                method: "GET",
        ...
        function push_select_mahasiswa(url, element) {
            $.ajax({
                url: url,
                method: "GET",
                success: function(res) {
        ...
        $(function() {
            initDatatable();
            push_select_dosen('/admin/registrasi-get', '#from_select_dosen');
            push_select_mahasiswa('/admin/mahasiswa-get', '#from_select_mahasiswa');
            control.push_select3(data, '#from_select_jenis')
            control.push_select3(pembimbing, '#from_select_pembimbing')
        });
    </script>
@endsection
